...
public class Fatigue {
	...
	private static int solution(int k, int[][] d) {
		...
		for (int i = 0; i < d.length; i++) {
			if (!visited[i] && k >= d[i][0]) {
				visited[i] = true;
				max = Math.max(max, dfs(visited, d, k - d[i][1], count + 1));
				visited[i] = false;
			}
		}
	...
	private static int dfs(boolean[] visited, int[][] d, int k, int count) {
		...
		for (int i = 0; i < d.length; i++) {
			...
			if (!visited[i] && k >= d[i][0]) {
				visited[i] = true;
				max = Math.max(dfs(visited, d, k - d[i][1], count + 1), max);
				visited[i] = false;
			}
		}
		return max;
