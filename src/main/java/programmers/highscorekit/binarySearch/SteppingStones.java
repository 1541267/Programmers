...
public class SteppingStones {
	...
	private static int solution(int d, int[] rocks, int n) {
		...
		while (left <= right) {
			...
			if (checkDistance(rocks, mid, d, n)) {
				result = mid;
				left = mid + 1;
			} else {
				right = mid - 1;
	...
	private static boolean checkDistance(int[] rocks, int mid, int d, int n) {
