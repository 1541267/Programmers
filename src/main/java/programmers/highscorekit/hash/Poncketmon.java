...
// 폰켓몬 당신은 폰켓몬을 잡기 위한 오랜 여행 끝에, 홍 박사님의 연구실에 도착했습니다.
...
public class Poncketmon {
	...
	private static int solution(int[] nums) {
		...
		HashSet<Integer> poncketMonList = new HashSet<>();

		for(Integer num : nums) {
			poncketMonList.add(num);
		}
		System.out.println("poncketMonList = " + poncketMonList);

		int totalSize = poncketMonList.size();

		return Math.min(totalSize, maxSelectCount);
	}
}
