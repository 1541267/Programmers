...
public class Process {
	...
	public Process(int process, int index) {
	...
	public static int solution(int[] priorities, int location) {
		...
		for (int i = 0; i < priorities.length; i++) {
			processList.offer(new Process(priorities[i], i));
		}

		while (!processList.isEmpty()) {

			Process currentProcess = processList.poll();

			boolean hasHigherProcessCheck = false;

			for (Process p : processList) {
				if (currentProcess.process < p.process) {
					hasHigherProcessCheck = true;
					break;
				}
			}

			if (hasHigherProcessCheck) {
